"""
Schema.org JSON-LD structured data for property landing pages.

Created 2026-08-19 (PAGE-2). Logic-preserving move from
agents/agent5/schema_markup.py. No database access, no agents/ imports.
"""
...
def generate_schema_jsonld(
    ...
    """Generate Schema.org JSON-LD block. Returns an HTML <script> tag string."""
    schema = {
...
def build_schema_from_inputs(
    ...
    """Convenience wrapper: extracts fields from Agent outputs and calls generate_schema_jsonld."""
    def _val(obj, key):
        f = obj.get(key)
    ...
    g_rating      = _val(kb, "airbnb_rating")
