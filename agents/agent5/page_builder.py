...
# Valid LLM curation categories — must match SubjectCategory enum in agent3/models.py.
# Used to validate llm_category values before applying them; invalid values fall back
# to GCV subject_category.
_CURATION_VALID_CATEGORIES: frozenset = frozenset({
...
# Roles that qualify an image for the Photo Tour (not just gallery).
_PHOTO_TOUR_ROLES: frozenset = frozenset({"hero", "supporting"})
...
<body class="vibe-{vibe_profile.replace('_','-')}">
  ...
  {_build_lightbox_gallery_js(gallery_items)}
...
def _gallery_items_from_curation(
    ...
    for result in per_image:
        ...
        if llm_cat in _CURATION_VALID_CATEGORIES:
            ...
            gcv_cat = (asset.get("subject_category") or "uncategorised").lower()
            category = gcv_cat if gcv_cat in _CURATION_VALID_CATEGORIES else "uncategorised"
            n_cat_corrected += 1
            logger.debug(
                "[Agent 5] Curation: invalid llm_category %r for %s — using GCV %r",
                llm_cat, asset_id[-30:], category,
            )
...
def _build_lightbox_gallery_js(gallery_items: list) -> str:
    ...
    gallery_items: list of dicts with url, alt, category keys.
    """
    if not gallery_items:
        return ""

    # Build JS array of photo objects
    photos_js_entries = []
    for item in gallery_items:
        url = _esc_js(item.get("url") or "")
        alt = _esc_js(item.get("alt") or "")
        cat = _esc_js(item.get("category") or "uncategorised")
        photos_js_entries.append(f"{{'url':'{url}','alt':'{alt}','cat':'{cat}'}}")
    ...
    cat_labels = {
        ...
        "outdoor_entertaining": "Outdoor",
        "living_room": "Living Room",
        "kitchen": "Kitchen",
        "master_bedroom": "Master Bedroom",
        "standard_bedroom": "Bedrooms",
        "bathroom": "Bathrooms",
        "game_entertainment": "Entertainment",
        "local_area": "Local Area",
        "uncategorised": "Other",
    }
    cat_tabs_js_entries = []
    for cat in seen_cats:
        label = _esc_js(cat_labels.get(cat, cat.replace("_", " ").title()))
        cat_tabs_js_entries.append(f"{{'key':'{_esc_js(cat)}','label':'{label}'}}")
